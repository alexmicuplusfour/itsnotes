...
wsl rsync -az --delete `
    ...
    --exclude='*.zip' --exclude='*.sql' --exclude='deploy*.ps1' --exclude='docker-compose.override.yml' `
...
ssh -i $SSH_KEY -o StrictHostKeyChecking=no $SERVER "cd $REMOTE && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d"
